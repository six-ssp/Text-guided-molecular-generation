...
def get_parallel_jobs():
    return max(1, int(os.environ.get("SDVAE_N_JOBS", "1")))
...
def parse(chunk, grammar):
    ...
    if n_jobs == 1:
        ...
        result_list = Parallel(n_jobs=n_jobs)(
            delayed(parse_many)(chunk[i: i + size], grammar) for i in range(0, len(chunk), size)
        )
...
def batch_decode(raw_logits, use_random, decode_times):
    ...
    if n_jobs == 1:
        ...
        result_list = Parallel(n_jobs=n_jobs)(
            delayed(decode_chunk)(logit_lists[i], use_random, decode_times) for i in range(len(logit_lists))
        )
    return [_1 for _0 in result_list for _1 in _0]
    ...
    def __init__(self, *args, **kwargs):
        ...
        if cmd_args.mode == 'cpu':
            self.ae.load_state_dict(
                torch.load(cmd_args.saved_model, map_location=lambda storage, loc: storage)
            )
        else:
